...
CFG = SimpleNamespace(
    dataset=SimpleNamespace(
        name='planar',
        datadir='./datasets/planar/planar_pyg/',
        filter=True,
    ),
...
if __name__ == '__main__':
    ...
    for split in dataset_split:
        ...
        xs_data = np.memmap(f'planar/{split}/xs.bin', dtype=np.int16, mode='w+', shape=xs.shape)
        xs_data[:] = xs.astype(np.int16)
        xs_data.flush()

        edge_indices_data = np.memmap(f'planar/{split}/edge_indices.bin', dtype=np.int16, mode='w+', shape=edge_indices.shape)
        edge_indices_data[:] = edge_indices.astype(np.int16)
        edge_indices_data.flush()

        edge_attrs_data = np.memmap(f'planar/{split}/edge_attrs.bin', dtype=np.int16, mode='w+', shape=edge_attrs.shape)
    ...
    json.dump(data_meta, open('planar/data_meta.json', 'w'))